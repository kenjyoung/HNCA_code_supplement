...
class binary_hidden_layer():
	...
	def forward(self, x):
		self.last_input = x
		self.last_output, self.key, self.last_fr = jit_binary_forward(x,self.theta, self.b, self.key)
		if(self.centered):
...
class softmax_output_layer():
	def __init__(self, key, num_outputs, input_length, lr, use_adam=False):
		self.key, subkey = jx.random.split(key)
	...
	def forward(self,x):
		self.last_input = x
		self.last_output, self.key, self.last_p = jit_softmax_forward(x, self.theta, self.b, self.key)
		return self.last_output
...
parser.add_argument("--output", "-o", type=str, default="REINFORCE.out")
parser.add_argument("--model", "-m", type=str, default="REINFORCE.model")
...
output_layer = softmax_output_layer(layer_keys[-1],num_classes,units[-1],alpha,use_adam)
...
def compute_binary_update_expanded(i, o, fr, R):
	grads = jit_binary_grads(fr,i)
	output_prob = fr*o+(1-fr)*(1-o)
	grad_estimator = -jnp.expand_dims(R,axis=1)*(2*o-1)/output_prob
	theta_grad = grads[0]*jnp.expand_dims(grad_estimator, axis=2)
	b_grad = grads[1]*grad_estimator
	theta_update = jnp.mean(theta_grad, axis=0)
	b_update = jnp.mean(b_grad, axis=0)
...
def compute_binary_update(l,R):
	return jit_compute_binary_update_expanded(l.last_input, l.last_output, l.last_fr, R)
...
def update(y_hat,y,baseline):
	...
	for h in reversed(hidden_layers):
		theta_update, b_update, theta_grad, b_grad = compute_binary_update(h,R-baseline)
