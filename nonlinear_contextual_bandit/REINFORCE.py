...
def nonlinear_loss(params, network, x, o, reward):
	fr_logit = network(params, x)
	output_logit = fr_logit*(2*o-1)
	loss = -jnp.sum(jx.nn.log_sigmoid(output_logit),axis=1)*reward
	return loss
...
class binary_network():
	...
	def backward(self, reward):
		...
		return self.loss_grad(self.params(), self.apply, self.last_input, self.last_output, reward)
...
class softmax_output_layer():
	...
	def forward(self,x):
		self.last_input = x
		self.last_output, self.key = self._forward(self.params(), self.apply, x, self.key)
		return self.last_output
...
def backward(y_hat,y):
	...
	output_grads = output_layer.backward(R)
	update = tree_mean(output_grads)
	output_layer.update(update)

	network_grads = network.backward(R)
