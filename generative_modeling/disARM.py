...
def forward(x, theta, b, key, sample=True):
	...
	if(sample):
		...
		u = jx.random.uniform(key=subkey, shape=[x.shape[0],theta.shape[0]])
		output = ((1-u)<fr).astype(float)
		anti_output = (u<fr).astype(float)
	else:
		output = None
		anti_output = None
	return output, anti_output, key, fr_logit, fr
...
class binary_hidden_layer():
	...
	def forward(self, x, sample=True):
		self.last_input = x
		self.last_output, self.last_anti_output, self.key, self.last_fr_logit, self.last_fr = jit_forward(x,self.theta, self.b, self.key, sample)
		if(sample):
...
parser.add_argument("--output", "-o", type=str, default="disARM.out")
parser.add_argument("--model", "-m", type=str, default="disARM.model")
...
#input considered as first layer
...
#this is used for computing reward for antithetic samples
def forward_R(x,i):
	first = True
	...
	for q, p in zip(encoder_layers[i:], list(reversed(decoder_layers))[i:]):
		...
		_, _, _, _, decoder_fr = jit_forward(x,p.theta, p.b, p.key, False)
...
def compute_updates_expanded(q_i, p_i, p_fr, p_fr_logit, q_fr, q_o, q_a, R, anti_R):
	...
	delta = R+jnp.sum(jnp.log(p_prob),axis=1)-anti_R

	q_grad_estimator = -(q_entropy_grad+0.5*jnp.expand_dims(delta,axis=1)*(q_o-q_a)/jnp.minimum(q_fr,1-q_fr))
...
def compute_updates(p, q, R, anti_R):
	p_i = p.last_input
	q_i = q.last_input
	p_fr = p.last_fr
	q_fr = q.last_fr
	...
	q_o = q.last_output
	q_a = q.last_anti_output
	return jit_compute_updates_expanded(q_i, p_i, p_fr, p_fr_logit, q_fr, q_o, q_a, R, anti_R)
...
def update():
	...
	for q, p in zip(reversed(encoder_layers), decoder_layers):
		anti_R = forward_R(q.last_anti_output, encoder_layers.index(q))
		p_theta_update, p_b_update, q_theta_update, q_b_update, q_theta_grad, q_b_grad, q_H, log_p_prob = compute_updates(p, q, R, anti_R)
