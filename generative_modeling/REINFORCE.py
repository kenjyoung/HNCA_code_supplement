...
def compute_updates_expanded(q_i, p_i, p_fr, q_fr, q_o, R):
	...
	q_grad_estimator = -(q_entropy_grad+jnp.expand_dims(R+jnp.sum(jnp.log(p_prob),axis=1),axis=1)*(2*q_o-1)/q_prob)
...
def compute_updates(p, q, R):
	p_i = p.last_input
	q_i = q.last_input
	p_fr = p.last_fr
	q_fr = q.last_fr
	q_o = q.last_output
	return jit_compute_updates_expanded(q_i, p_i, p_fr, q_fr, q_o, R)
...
def update():
	#Input x is a latent space encodeing
	...
	for i, (q, p) in enumerate(zip(reversed(encoder_layers), decoder_layers)):
		p_theta_update, p_b_update, q_theta_update, q_b_update, q_theta_grad, q_b_grad, q_H, log_p_prob = compute_updates(p, q, R-baselines[i])
		...
		if(use_baseline):
			baselines[i] = gamma*baselines[i]+(1-gamma)*jnp.mean(R+log_p_prob)
		#accumulate reward
		R += log_p_prob+q_H
